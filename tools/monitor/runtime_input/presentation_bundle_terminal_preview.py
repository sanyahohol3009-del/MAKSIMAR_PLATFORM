...
def main() -> None:
    ...
    print("PRESENTATION BUNDLE PREVIEW")
    print("=" * 180)

    for entry in contract.entries:
        print(
            f"{entry.presentation_bundle_id:<24} | "
            f"{entry.display_target_id:<30} | "
            f"{entry.presentation_bundle_class:<32}"
        )
        print(
            " " * 5
        ...
        print(
            " " * 5
            + f"presentation_bundle_state={entry.presentation_bundle_state} | "
            f"dashboard_visible_state_ready={entry.dashboard_visible_state_ready} | "
            f"display_mapping_consistent={entry.display_mapping_consistent}"
        )
        print(
            " " * 5
