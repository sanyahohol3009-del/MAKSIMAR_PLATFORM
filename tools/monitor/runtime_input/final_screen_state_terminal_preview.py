...
def main() -> None:
    ...
    print("FINAL SCREEN STATE PREVIEW")
    print("=" * 180)

    for entry in contract.entries:
        print(
            f"{entry.final_screen_state_id:<24} | "
            f"{entry.display_target_id:<30} | "
            f"{entry.final_screen_state_class:<28}"
        )
        print(
            " " * 5
            + f"workspace_id={entry.workspace_id} | "
            f"final_screen_state_state={entry.final_screen_state_state} | "
            f"final_screen_state_mode={entry.final_screen_state_mode}"
        )
        print(
            " " * 5
            + f"final_visible_screen_state_ready={entry.final_visible_screen_state_ready} | "
            f"presentation_bundle_runtime_ready={entry.presentation_bundle_runtime_ready}"
        )
        print(
            " " * 5
    ...
    print(
        ...
        f"foundation_final_entries={contract.foundation_final_entries} | "
        f"interaction_final_entries={contract.interaction_final_entries} | "
