...
## 40. VERIFIED PATCH V26 — C2.12f-REV1 PERMANENT DASHBOARD NAVIGATION RAIL READ-MODEL

### STATUS
C2.12f-REV1 permanent dashboard navigation rail read-model completed.

### PURPOSE
Correct the previous left-drawer navigation wording and create the canonical persistent dashboard navigation rail model.

### ADDED READ-MODEL
- frontend/react_flow_preview/src/permanentDashboardNavigationRailReadModel.ts

### ADDED TEST
- frontend/tests/permanentDashboardNavigationRailReadModel.test.ts

### CONFIRMED RAIL MODEL
The permanent dashboard navigation rail exposes:
- 19 dashboard sections
- 34 dashboard items
- persistent left rail display mode
- memory dashboard entries
- mobile companion entries
- 3D / simulation adapter-boundary entries
- renderer adapter identifiers
- approval / status / adapter badges

### SEMANTIC CORRECTION
The main dashboard list is not a temporary drawer.

The permanent left rail is the stable navigation root for dashboard selection.

### ACTIVE DASHBOARD DRAWER SEPARATION
Drawer panels are reserved for active-dashboard-specific content:
- left contextual drawer = functions / settings / tools of selected dashboard
- right contextual drawer = state / diagnostics / context of selected dashboard

### APP.TSX RULE
Dashboard rail buttons must be consumed from this read-model in a future visual wiring step.

They must not be hardcoded manually in App.tsx.
