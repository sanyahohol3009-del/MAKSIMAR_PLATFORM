...
from ANDROID_SHELL.screen_observer_client.android_screen_consent_state import (
    AndroidScreenConsentState,
)


def test_android_screen_consent_state_smoke() -> None:
    state = AndroidScreenConsentState(
...
def test_android_screen_consent_state_rejects_permission_prompt() -> None:
...
def test_android_screen_consent_state_rejects_platform_api_call() -> None:
    with pytest.raises(ValueError, match="android_platform_api_call_allowed must be False"):
        AndroidScreenConsentState(
            device_id="android_device_001",
            owner_identity_id="owner_001",
            session_id="android_screen_session_001",
            ...
            audit_required=True,
            permission_prompt_allowed=False,
            android_platform_api_call_allowed=True,
            runtime_mutation_allowed=False,
            core_write_allowed=False,
            source_of_truth_override_allowed=False,
        )
