...
from ANDROID_SHELL.p2p_node_adapter.p2p_node_state_bridge import (
    AndroidP2PNodeStateBridge,
    build_android_p2p_node_state_bridge,
)


def test_android_p2p_node_state_bridge_smoke() -> None:
    bridge = build_android_p2p_node_state_bridge(server_present=True)
    floating = build_android_floating_master_state(server_present=False)

    assert isinstance(bridge, AndroidP2PNodeStateBridge)
    assert isinstance(floating, AndroidFloatingMasterState)
    assert bridge.platform == "android"
    assert bridge.node_role == "android_node"
