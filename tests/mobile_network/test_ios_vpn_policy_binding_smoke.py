...
from IOS_SHELL.network_vpn.ios_vpn_policy_binding import (
    IosVpnPolicyBinding,
    build_ios_vpn_policy_binding,
)


def test_ios_vpn_policy_binding_smoke() -> None:
    binding = build_ios_vpn_policy_binding()

    assert isinstance(binding, IosVpnPolicyBinding)
    assert binding.platform == "ios"
    ...
    assert binding.shell_layer == "IOS_SHELL/network_vpn"
