...
def test_ios_vpn_profile_contract_smoke() -> None:
    profile = build_ios_vpn_profile_model()

    assert isinstance(profile, IosVpnProfileModel)
    ...
    assert profile.profile_editable_from_ios is False
    assert profile.system_api_call_allowed is False
    assert profile.network_extension_api_call_allowed is False
    assert profile.nevpn_api_call_allowed is False
