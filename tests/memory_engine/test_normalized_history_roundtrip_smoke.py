...
def test_normalized_history_roundtrip_smoke() -> None:
    segment = build_segment(
        parent_document_id="HDOC-0005",
        source_type="json",
    ...
    record = build_normalized_history_record(segment, 4)

    write_payload = build_normalized_history_write_payload(record)
    read_payload = build_normalized_history_read_payload(record)

    assert write_payload["memory_id"] == read_payload["memory_id"]
    assert write_payload["write_path"] == read_payload["write_path"]
