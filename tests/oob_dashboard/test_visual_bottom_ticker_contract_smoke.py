...
def test_visual_bottom_ticker_log_counts_are_truthful() -> None:
    """Visual bottom ticker log counts should be derived from logs content."""
    contract = build_visual_bottom_ticker_contract()
    entry = contract.entries[0]

    assert entry.total_log_sources == 4
    assert entry.active_log_sources == 2
    assert entry.highlighted_log_sources == 2
