...
def test_visual_status_bar_runtime_counts_are_truthful() -> None:
    """Visual status bar runtime counts should be derived from system status content."""
    contract = build_visual_status_bar_contract()
    entry = contract.entries[0]

    assert entry.total_runtime_surfaces == 4
    assert entry.active_runtime_surfaces == 3
    assert entry.warning_runtime_surfaces == 1
