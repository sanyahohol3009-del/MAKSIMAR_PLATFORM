...
def test_base_family_manifest_contract_builds() -> None:
    contract = build_base_family_manifest_contract()
    assert contract.contract_id == "base_family_manifest_contract_001"
    assert contract.total_entries == 2
    assert contract.mount_required_entries == 2
