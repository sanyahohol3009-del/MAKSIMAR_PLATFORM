...
def _require_true(value: bool, field_name: str) -> None:
    if value is not True:
        raise ValueError(f"{field_name} must be True in iOS workflow permission bridge")


def _require_false(value: bool, field_name: str) -> None:
    if value is not False:
        raise ValueError(f"{field_name} must be False in iOS workflow permission bridge")


@dataclass(frozen=True)
class IOSWorkflowPermissionBridgeResult:
    bridge_id: str
    platform: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.platform != IOS_WORKFLOW_PLATFORM:
...
@dataclass(frozen=True)
class IOSWorkflowPermissionBridge:
    bridge_id: str
    capability_limits: IOSWorkflowCapabilityLimits
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "bridge_id", _require_non_empty_text(self.bridge_id, "bridge_id"))
        if not isinstance(self.capability_limits, IOSWorkflowCapabilityLimits):
            raise TypeError("capability_limits must be an IOSWorkflowCapabilityLimits")
    ...
    def evaluate_proposal(self, proposal: LocalAIWorkflowProposalContract) -> IOSWorkflowPermissionBridgeResult:
        ...
        return IOSWorkflowPermissionBridgeResult(
            bridge_id=self.bridge_id,
...
def build_ios_workflow_permission_bridge(
    *,
    bridge_id: str = "ios.workflow.permission.bridge.v1",
    capability_limits: Optional[IOSWorkflowCapabilityLimits] = None,
    explicit_user_permission_granted: bool,
    device_owner_confirmed: bool,
) -> IOSWorkflowPermissionBridge:
    return IOSWorkflowPermissionBridge(
        bridge_id=bridge_id,
        capability_limits=capability_limits or build_ios_workflow_capability_limits(),
...
__all__ = [
    "IOSWorkflowPermissionBridge",
    "IOSWorkflowPermissionBridgeResult",
