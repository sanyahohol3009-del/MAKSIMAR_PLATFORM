...
from IOS_SHELL.network_vpn.vpn_sync_contract import (
    IosVpnSyncContract,
    build_ios_vpn_sync_contract,
)


@dataclass(frozen=True, slots=True)
class IosVpnPolicyBinding:
    """iOS VPN shell policy binding.

    Binds iOS shell VPN projection to canonical network security policy.
    """
    ...
    state_bridge: IosVpnStateBridge
    sync_contract: IosVpnSyncContract
    vpn_policy: VpnPolicyDisableContract
    egress_policy: EgressPolicyContract
    ios_shell_binding_ready: bool
    ...
    def __post_init__(self) -> None:
        if self.schema_version != "ios_vpn_policy_binding.v1":
            raise ValueError("schema_version must be ios_vpn_policy_binding.v1")
        if self.binding_id != "ios_vpn_policy_binding_disabled_default":
            raise ValueError("binding_id must be ios_vpn_policy_binding_disabled_default")
        ...
        if not isinstance(self.state_bridge, IosVpnStateBridge):
            raise TypeError("state_bridge must be IosVpnStateBridge")
        if not isinstance(self.sync_contract, IosVpnSyncContract):
            raise TypeError("sync_contract must be IosVpnSyncContract")
        ...
        if self.shell_layer != "IOS_SHELL/network_vpn":
            raise ValueError("shell_layer must be IOS_SHELL/network_vpn")
        if not isinstance(self.reason_codes, tuple) or not self.reason_codes:
            raise ValueError("reason_codes must be a non-empty tuple")

        required_true = {
            "ios_shell_binding_ready": self.ios_shell_binding_ready,
...
def build_ios_vpn_policy_binding() -> IosVpnPolicyBinding:
    return IosVpnPolicyBinding(
        schema_version="ios_vpn_policy_binding.v1",
        binding_id="ios_vpn_policy_binding_disabled_default",
        platform="ios",
        profile=build_ios_vpn_profile_model(),
        permission_state=build_ios_vpn_permission_state(),
        state_bridge=build_ios_vpn_state_bridge(),
        sync_contract=build_ios_vpn_sync_contract(),
        vpn_policy=build_default_vpn_policy_disable_contract(),
        egress_policy=build_default_egress_policy_contract(),
        ios_shell_binding_ready=True,
        source_of_truth_layer="MAKSIMAR_CORE_LIB/network_security",
        runtime_layer="MAKSIMAR_SERVER/NETWORK_SECURITY_RUNTIME",
        shell_layer="IOS_SHELL/network_vpn",
        ...
        reason_codes=(
            "ios_vpn_shell_binding_ready",
            "canonical_network_security_policy_reused",
            "no_ios_network_extension_execution",
            "no_nevpn_api_execution",
        ),
    )
