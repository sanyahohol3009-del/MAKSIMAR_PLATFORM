...
class IOSOfflineQueueBridgeEntry:
    delivery_id: str
    message_id: str
    target_room_id: str
    queue_state: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.background_task_allowed:
            raise ValueError("background_task_allowed must be False")
...
@dataclass
class IOSOfflineQueueBridge:
    """In-memory iOS offline queue bridge."""

    _entries: Dict[str, IOSOfflineQueueBridgeEntry] = field(default_factory=dict)

    def enqueue(self, entry: IOSOfflineQueueBridgeEntry) -> IOSOfflineQueueBridgeEntry:
    ...
    def mark_waiting_for_server(self, delivery_id: str) -> IOSOfflineQueueBridgeEntry:
        current = self.get_entry(delivery_id)
        updated = IOSOfflineQueueBridgeEntry(
    ...
    def get_entry(self, delivery_id: str) -> IOSOfflineQueueBridgeEntry:
