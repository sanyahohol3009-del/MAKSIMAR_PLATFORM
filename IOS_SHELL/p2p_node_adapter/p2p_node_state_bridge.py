...
class IosP2PNodeStateBridge:
    """iOS shell bridge over shared P2P/Floating Master contracts."""
    ...
    floating_master_state: IosFloatingMasterState
    bridge_ready: bool
    node_role: str
    source_layer: str
    ...
    def __post_init__(self) -> None:
        if self.schema_version != "ios_p2p_node_state_bridge.v1":
            raise ValueError("schema_version must be ios_p2p_node_state_bridge.v1")
        if self.bridge_id != "ios_p2p_node_state_bridge_disabled_default":
            raise ValueError("bridge_id must be ios_p2p_node_state_bridge_disabled_default")
        ...
        if self.node_role != "ios_node":
            raise ValueError("node_role must be ios_node")
        ...
        if not isinstance(self.floating_master_state, IosFloatingMasterState):
            raise TypeError("floating_master_state must be IosFloatingMasterState")
...
def build_ios_p2p_node_state_bridge(server_present: bool = True) -> IosP2PNodeStateBridge:
    return IosP2PNodeStateBridge(
        schema_version="ios_p2p_node_state_bridge.v1",
        bridge_id="ios_p2p_node_state_bridge_disabled_default",
        platform="ios",
        ...
        floating_master_state=build_ios_floating_master_state(server_present=server_present),
        bridge_ready=True,
        node_role="ios_node",
        ...
        reason_codes=("ios_p2p_node_adapter_projects_shared_p2p_state",),
    )
