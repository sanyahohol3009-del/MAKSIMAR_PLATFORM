...
ROLLBACK_MANAGER_RUNTIME_ID = "rollback_manager_runtime_v1"


@dataclass(frozen=True, slots=True)
class RollbackManagerRuntimeResult:
    runtime_id: str
    package_id: str
    read_model: RollbackManagerReadinessReadModel
    wrapper_only: bool
    ...
    def __post_init__(self) -> None:
        if self.runtime_id != ROLLBACK_MANAGER_RUNTIME_ID:
            raise ValueError("runtime_id must be rollback_manager_runtime_v1")
        _validate_non_empty("package_id", self.package_id)
        if not isinstance(self.read_model, RollbackManagerReadinessReadModel):
            raise TypeError("read_model must be RollbackManagerReadinessReadModel")
...
def run_rollback_manager_ready(
    *,
    ...
    return RollbackManagerRuntimeResult(
        runtime_id=ROLLBACK_MANAGER_RUNTIME_ID,
        package_id=package_id,
        ...
        reason_codes=("rollback_manager_runtime_wrapped_contract",),
    )


def run_rollback_manager_blocked(
    *,
    package_id: str,
    snapshot_readiness: SnapshotManagerReadinessReadModel,
    reason_codes: tuple[str, ...],
    ...
    return RollbackManagerRuntimeResult(
        ...
        canonical_write_allowed=False,
        dashboard_execution_allowed=False,
        reason_codes=("rollback_manager_runtime_wrapped_blocked_contract",),
    )
