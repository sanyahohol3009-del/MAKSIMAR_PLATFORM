...
RECOVERY_SERVICE_RUNTIME_ID = "recovery_service_runtime_v1"


@dataclass(frozen=True, slots=True)
class RecoveryServiceRuntimeResult:
    runtime_id: str
    package_id: str
    read_model: RecoveryServiceReadinessReadModel
    wrapper_only: bool
    ...
    def __post_init__(self) -> None:
        if self.runtime_id != RECOVERY_SERVICE_RUNTIME_ID:
            raise ValueError("runtime_id must be recovery_service_runtime_v1")
        _validate_non_empty("package_id", self.package_id)
        if not isinstance(self.read_model, RecoveryServiceReadinessReadModel):
            raise TypeError("read_model must be RecoveryServiceReadinessReadModel")
...
def run_recovery_service_ready(
    *,
    ...
    return RecoveryServiceRuntimeResult(
        ...
        reason_codes=("recovery_service_runtime_wrapped_contract",),
    )


def run_recovery_service_blocked(
    *,
    package_id: str,
    snapshot_readiness: SnapshotManagerReadinessReadModel,
    rollback_readiness: RollbackManagerReadinessReadModel,
    ...
    return RecoveryServiceRuntimeResult(
        ...
        canonical_write_allowed=False,
        dashboard_execution_allowed=False,
        reason_codes=("recovery_service_runtime_wrapped_blocked_contract",),
    )
