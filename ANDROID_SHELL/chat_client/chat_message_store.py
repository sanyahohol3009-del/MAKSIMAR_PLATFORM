...
class AndroidChatMessageStoreEntry:
...
@dataclass
class AndroidChatMessageStore:
    """Local in-memory Android message store contract.

    It stores references/previews only. It does not persist plaintext, write
    canonical truth, or send data over the network.
    """

    _entries: Dict[str, AndroidChatMessageStoreEntry] = field(default_factory=dict)

    def add_entry(self, entry: AndroidChatMessageStoreEntry) -> AndroidChatMessageStoreEntry:
    ...
    def get_entry(self, local_message_id: str) -> AndroidChatMessageStoreEntry:
    ...
    def list_entries(self) -> Tuple[AndroidChatMessageStoreEntry, ...]:
