...
class AndroidVpnStateBridge:
    """Android shell bridge for VPN status.

    Bridges canonical disabled/read-only VPN state into Android shell projection.
    """
    ...
    def __post_init__(self) -> None:
        if self.schema_version != "android_vpn_state_bridge.v1":
            raise ValueError("schema_version must be android_vpn_state_bridge.v1")
        if self.bridge_id != "android_vpn_state_bridge_disabled_default":
            raise ValueError("bridge_id must be android_vpn_state_bridge_disabled_default")
        ...
        if self.mobile_hook.platform != "android":
            raise ValueError("mobile_hook must be android hook")
...
def _get_android_mobile_hook() -> MobileVpnHookContract:
    for hook in build_default_mobile_vpn_hooks():
        if hook.platform == "android":
            return hook
    raise RuntimeError("android mobile VPN hook is missing")


def build_android_vpn_state_bridge() -> AndroidVpnStateBridge:
    return AndroidVpnStateBridge(
        schema_version="android_vpn_state_bridge.v1",
        bridge_id="android_vpn_state_bridge_disabled_default",
        platform="android",
        profile=build_android_vpn_profile_model(),
        permission_state=build_android_vpn_permission_state(),
        mobile_hook=_get_android_mobile_hook(),
        ...
        reason_codes=("android_vpn_state_bridge_projects_canonical_disabled_vpn_state",),
    )
