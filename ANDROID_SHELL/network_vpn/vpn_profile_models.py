...
class AndroidVpnProfileModel:
    """Android shell projection of the canonical mobile VPN profile."""

    schema_version: str
    profile_id: str
    android_profile_id: str
    canonical_profile: VpnProfileContract
    shell_surface: str
    platform: str
    profile_visible: bool
    profile_editable_from_android: bool
    ...
    def __post_init__(self) -> None:
        if self.schema_version != "android_vpn_profile_model.v1":
            raise ValueError("schema_version must be android_vpn_profile_model.v1")
        if self.platform != "android":
            raise ValueError("platform must be android")
        if self.shell_surface != "ANDROID_SHELL/network_vpn":
            raise ValueError("shell_surface must be ANDROID_SHELL/network_vpn")
        ...
        if self.canonical_profile.profile_id != "vpn_mobile_profile":
            raise ValueError("Android VPN profile must bind to vpn_mobile_profile")
        ...
        required_false = {
            "profile_editable_from_android": self.profile_editable_from_android,
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "android_profile_id": self.android_profile_id,
...
def build_android_vpn_profile_model() -> AndroidVpnProfileModel:
    return AndroidVpnProfileModel(
        schema_version="android_vpn_profile_model.v1",
        profile_id="vpn_mobile_profile",
        android_profile_id="android_vpn_mobile_profile_projection",
        canonical_profile=_get_mobile_profile(),
        shell_surface="ANDROID_SHELL/network_vpn",
        platform="android",
        profile_visible=True,
        profile_editable_from_android=False,
        ...
        reason_codes=("android_vpn_profile_projection_uses_canonical_mobile_profile",),
    )
