...
ANDROID_WORKFLOW_PLATFORM = "android"
ALLOWED_ANDROID_WORKFLOW_SCOPES: Tuple[str, ...] = (
    "local_app_workflow",
    "device_assisted_workflow",
)
DEFAULT_ANDROID_WORKFLOW_CAPABILITIES: Tuple[str, ...] = (
...
def _require_true(value: bool, field_name: str) -> None:
    if value is not True:
        raise ValueError(f"{field_name} must be True in Android workflow capability limits")


def _require_false(value: bool, field_name: str) -> None:
    if value is not False:
        raise ValueError(f"{field_name} must be False in Android workflow capability limits")


@dataclass(frozen=True)
class AndroidWorkflowCapabilityLimits:
    limits_id: str
    allowed_capability_refs: Tuple[str, ...]
    platform_capability_refs: Tuple[str, ...]
    allowed_workflow_scopes: Tuple[str, ...] = ALLOWED_ANDROID_WORKFLOW_SCOPES
    ...
    def __post_init__(self) -> None:
        ...
        unknown_scopes = tuple(scope for scope in self.allowed_workflow_scopes if scope not in ALLOWED_ANDROID_WORKFLOW_SCOPES)
        if unknown_scopes:
            raise ValueError(f"allowed_workflow_scopes contains unsupported Android scopes: {unknown_scopes}")
...
def build_android_workflow_capability_limits(
    *,
    limits_id: str = "android.workflow.capability.limits.v1",
    revoked_capability_refs: Tuple[str, ...] = (),
) -> AndroidWorkflowCapabilityLimits:
    return AndroidWorkflowCapabilityLimits(
        limits_id=limits_id,
        allowed_capability_refs=DEFAULT_ANDROID_WORKFLOW_CAPABILITIES,
        platform_capability_refs=DEFAULT_ANDROID_WORKFLOW_CAPABILITIES,
        revoked_capability_refs=revoked_capability_refs,
    )


__all__ = [
    "ALLOWED_ANDROID_WORKFLOW_SCOPES",
    "ANDROID_WORKFLOW_PLATFORM",
    "AndroidWorkflowCapabilityLimits",
    "DEFAULT_ANDROID_WORKFLOW_CAPABILITIES",
