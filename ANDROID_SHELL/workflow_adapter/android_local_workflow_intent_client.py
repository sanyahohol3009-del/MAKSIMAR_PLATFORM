...
ANDROID_LOCAL_WORKFLOW_PLATFORM = "android"
...
def _require_true(value: bool, field_name: str) -> None:
    if value is not True:
        raise ValueError(f"{field_name} must be True in Android local workflow intent client")


def _require_false(value: bool, field_name: str) -> None:
    if value is not False:
        raise ValueError(f"{field_name} must be False in Android local workflow intent client")


@dataclass(frozen=True)
class AndroidLocalWorkflowIntentDecision:
    local_intent_id: str
    client_id: str
    proposal_id: str
    graph_id: str
    ...
    audit_event_id: str
    platform: str = ANDROID_LOCAL_WORKFLOW_PLATFORM
    ...
    def __post_init__(self) -> None:
        ...
        if self.platform != ANDROID_LOCAL_WORKFLOW_PLATFORM:
...
@dataclass(frozen=True)
class AndroidLocalWorkflowIntentClient:
    client_id: str
    permission_bridge: AndroidWorkflowPermissionBridge
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "client_id", _require_non_empty_text(self.client_id, "client_id"))
        if not isinstance(self.permission_bridge, AndroidWorkflowPermissionBridge):
            raise TypeError("permission_bridge must be an AndroidWorkflowPermissionBridge")
        ...
        *,
    ...
    ) -> AndroidLocalWorkflowIntentDecision:
        ...
        if permission_result.permission_decision.allowed is not True:
            safety_decision = WorkflowSafetyDecision(
                ...
                reason="Android permission bridge denied the local workflow proposal",
        ...
        return AndroidLocalWorkflowIntentDecision(
...
def build_android_local_workflow_intent_client(
    *,
    client_id: str = "android.local.workflow.intent.client.v1",
    permission_bridge: AndroidWorkflowPermissionBridge,
    safety_policy: WorkflowSafetyPolicyContract | None = None,
) -> AndroidLocalWorkflowIntentClient:
    return AndroidLocalWorkflowIntentClient(
        client_id=client_id,
...
__all__ = [
    "ANDROID_LOCAL_WORKFLOW_PLATFORM",
    "AndroidLocalWorkflowIntentClient",
    "AndroidLocalWorkflowIntentDecision",
