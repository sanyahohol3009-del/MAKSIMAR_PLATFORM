...
class AndroidScreenObserverClient:
    device_id: str
    ...
    accessibility_service_allowed: bool
    ...
    def __post_init__(self) -> None:
        ...
        if self.device_type != "android":
            raise ValueError("device_type must be android")
        if not self.read_only:
            raise ValueError("normal Android observer must be read-only")
        ...
        if self.accessibility_service_allowed:
            raise ValueError("accessibility_service_allowed must be False")
        if self.remote_control_allowed:
            raise ValueError("remote_control_allowed must be False")
        if self.child_control_enabled:
            raise ValueError("normal Android observer cannot enable child control")
    ...
    @classmethod
    def default(cls, *, device_id: str, owner_identity_id: str) -> "AndroidScreenObserverClient":
        return cls(
            device_id=device_id,
    ...
    def to_read_model(self) -> dict[str, object]:
        return {
            ...
            "accessibility_service_allowed": self.accessibility_service_allowed,
            "remote_control_allowed": self.remote_control_allowed,
